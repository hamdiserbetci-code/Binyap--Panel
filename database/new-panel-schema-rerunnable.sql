drop index if exists idx_kullanicilar_firma;
drop index if exists idx_projeler_firma;
drop index if exists idx_ekipler_firma;
drop index if exists idx_ekipler_proje;
drop index if exists idx_puantaj_proje_tarih;
drop index if exists idx_gelir_proje_tarih;
drop index if exists idx_gider_proje_tarih;
drop index if exists idx_odeme_vade;
drop index if exists idx_kasa_tarih;
drop index if exists idx_vergi_firma_yil;
drop index if exists idx_dokuman_modul;
drop index if exists idx_gorevler_firma;
drop index if exists idx_bildirimler_firma;
drop index if exists idx_aktivite_loglari_firma;

do $$
begin
  if exists (
    select 1 from information_schema.tables
    where table_schema = ''public'' and table_name = ''kullanici_profilleri''
  ) then
    execute ''drop trigger if exists trg_kullanici_profilleri_defaults on public.kullanici_profilleri'';
    execute ''drop trigger if exists trg_kullanici_profilleri_guard on public.kullanici_profilleri'';
  end if;
end $$;

drop function if exists handle_profile_defaults() cascade;
drop function if exists guard_profile_update() cascade;
drop function if exists has_panel_permission(text) cascade;
drop function if exists current_app_role() cascade;
drop function if exists current_firma_id() cascade;
...
create index if not exists idx_kullanicilar_firma on kullanici_profilleri(firma_id, rol, aktif);
create index if not exists idx_projeler_firma on projeler(firma_id);
create index if not exists idx_ekipler_firma on ekipler(firma_id);
create index if not exists idx_ekipler_proje on ekipler(proje_id);
create index if not exists idx_puantaj_proje_tarih on puantaj_kayitlari(proje_id, tarih);
create index if not exists idx_gelir_proje_tarih on gelir_kayitlari(proje_id, tarih);
create index if not exists idx_gider_proje_tarih on gider_kayitlari(proje_id, tarih);
create index if not exists idx_odeme_vade on odeme_planlari(vade_tarihi);
create index if not exists idx_kasa_tarih on kasa_hareketleri(tarih);
create index if not exists idx_vergi_firma_yil on vergi_surecleri(firma_id, yil, surec_turu);
create index if not exists idx_dokuman_modul on dokumanlar(modul, kategori);
create index if not exists idx_gorevler_firma on gorevler(firma_id, durum, son_tarih);
create index if not exists idx_bildirimler_firma on bildirim_kayitlari(firma_id, kaynak_turu, kapali, erteleme_sonrasi);
...
create index if not exists idx_aktivite_loglari_firma on aktivite_loglari(firma_id, modul, created_at desc);
