  ...
  if (selectedBanka) return (
    <div>
      <div className="flex items-center gap-3 mb-4">
        <button onClick={() => setSelectedBanka(null)} className="w-8 h-8 rounded-xl border border-white/[0.08] flex items-center justify-center text-slate-400 hover:bg-white/[0.04]"><ArrowLeft size={14}/></button>
        <div className="flex-1">
          <h2 className="text-base font-semibold text-white">{selectedBanka.banka_adi}</h2>
          <p className="text-xs text-slate-400">{selectedBanka.sube||''} • Bakiye: <span className={selectedBanka.bakiye>=0?'text-emerald-400':'text-red-400'}>{fmt(selectedBanka.bakiye)}</span></p>
        </div>
      ...
      <div className="bg-white/[0.02] rounded-xl border border-white/[0.05] p-4 mb-4 grid grid-cols-2 sm:grid-cols-3 gap-3">
        {selectedBanka.hesap_no && <div><p className="text-[10px] text-slate-400">Hesap No</p><p className="text-sm text-slate-200">{selectedBanka.hesap_no}</p></div>}
        {selectedBanka.iban && <div className="col-span-2"><p className="text-[10px] text-slate-400">IBAN</p><p className="text-sm text-slate-200 font-mono">{selectedBanka.iban}</p></div>}
        <div><p className="text-[10px] text-slate-400">Para Birimi</p><p className="text-sm text-slate-200">{selectedBanka.para_birimi}</p></div>
      </div>
      ...
      {hareketler.length === 0 ? (
        ...
        <div className="bg-white/[0.02] rounded-xl border border-white/[0.05] overflow-hidden">
          <div className="grid grid-cols-4 gap-2 px-4 py-2.5 bg-white/[0.04] border-b border-white/[0.05] text-xs font-medium text-slate-400">
            <div>Tarih</div><div>Açıklama</div><div className="text-right">Tutar</div><div className="text-right">Bakiye</div>
          </div>
          {hareketler.map(h => (
            <div key={h.id} className="grid grid-cols-4 gap-2 px-4 py-3 border-b border-white/[0.02] last:border-0 text-sm hover:bg-white/[0.04]/50 group">
              ...
              <div className={`text-right font-medium ${h.tur==='giris'?'text-emerald-400':'text-red-400'}`}>{h.tur==='giris'?'+':'-'}{fmt(h.tutar)}</div>
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <div>
          <h2 className="text-base font-semibold text-white">Banka Hesapları</h2>
          <p className="text-xs text-slate-400 mt-0.5">{bankalar.length} hesap • Toplam: <span className={toplamBakiye>=0?'text-emerald-400':'text-red-400'}>{fmt(toplamBakiye)}</span></p>
        </div>
      ...
      {loading ? <p className="text-center text-slate-400 py-8 text-sm">Yükleniyor...</p> :
        bankalar.length === 0 ? (
          <div className="text-center py-12 bg-white/[0.02] rounded-xl border border-dashed border-white/[0.08]">
            ...
            <button onClick={() => openModal()} className="text-blue-400 text-sm hover:underline">İlk hesabı ekle</button>
          </div>
        ) : (
          <div className="space-y-2">
            {bankalar.map(b => (
              <div key={b.id} className="bg-white/[0.02] rounded-xl border border-white/[0.05] p-3.5 flex items-center gap-3 hover:border-white/[0.08] transition-all cursor-pointer group" onClick={() => { setSelectedBanka(b); fetchHareketler(b.id) }}>
                <div className="w-10 h-10 rounded-xl bg-emerald-500/10 flex items-center justify-center flex-shrink-0">
                  <span className="text-xs font-bold text-emerald-400">{b.banka_adi.slice(0,3).toUpperCase()}</span>
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-white">{b.banka_adi}</p>
                ...
                <div className="text-right flex-shrink-0">
                  <p className={`text-sm font-semibold ${b.bakiye>=0?'text-emerald-400':'text-red-400'}`}>{fmt(b.bakiye)}</p>
                ...
                <div className="flex gap-1 flex-shrink-0 opacity-0 group-hover:opacity-100" onClick={e=>e.stopPropagation()}>
                  <button onClick={() => openModal(b)} className="w-7 h-7 rounded-lg border border-white/[0.05] hover:bg-white/[0.04] flex items-center justify-center text-slate-400"><Pencil size={12}/></button>
                  <button onClick={() => handleDelete(b.id)} className="w-7 h-7 rounded-lg border border-white/[0.05] hover:bg-red-500/10 hover:border-red-200 flex items-center justify-center text-slate-400 hover:text-red-400"><Trash2 size={12}/></button>
                </div>
              </div>
            ))}
          </div>
