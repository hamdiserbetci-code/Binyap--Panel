...
export default function ProjelerPage({ userId, firma, onProjeSelect, selectedProjeId }: Props) {
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex items-center justify-between mb-3">
        <h3 className="text-sm font-semibold text-slate-700">Projeler</h3>
        ...
        {loading ? <p className="text-center text-slate-400 py-8 text-xs">Yükleniyor...</p> :
          projeler.length === 0 ? (
            ...
            <div key={p.id} onClick={() => onProjeSelect(p)}
              className={`bg-white rounded-xl border p-3 cursor-pointer hover:border-blue-300 transition-all flex items-center gap-2.5 ${selectedProjeId===p.id?'border-blue-400 bg-blue-50':'border-slate-100'}`}>
              <div className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 ${selectedProjeId===p.id?'bg-blue-600':'bg-slate-100'}`}>
                <FolderOpen size={14} className={selectedProjeId===p.id?'text-white':'text-slate-500'}/>
              </div>
              <div className="flex-1 min-w-0">
                <p className={`text-sm font-medium truncate ${selectedProjeId===p.id?'text-blue-700':'text-slate-800'}`}>{p.ad}</p>
                <span className={`text-[10px] px-1.5 py-0.5 rounded-full font-medium ${DC[p.durum]}`}>{DL[p.durum]}</span>
              </div>
              <div className="flex items-center gap-1 flex-shrink-0" onClick={e=>e.stopPropagation()}>
                <button onClick={()=>openModal(p)} className="w-6 h-6 rounded flex items-center justify-center text-slate-400 hover:text-slate-600"><Pencil size={10}/></button>
                <button onClick={()=>handleDelete(p)} className="w-6 h-6 rounded flex items-center justify-center text-slate-400 hover:text-red-500"><Trash2 size={10}/></button>
              </div>
