    ...
    <div className="flex flex-col h-full">
      ...
      <nav className="flex-1 overflow-y-auto py-3 px-3 space-y-1">
        {Object.entries(groups).map(([groupName, items]) => {
          const kapali = closedGroups.has(groupName)
          const aktifGrupMu = items.some(i => i.id === activeModule)
          return (
            <div key={groupName}>
              <button
                onClick={() => toggleGroup(groupName)}
                className={`w-full flex items-center justify-between px-3 py-1.5 rounded-lg mb-0.5 transition-colors group ${aktifGrupMu && kapali ? 'bg-blue-600/10' : 'hover:bg-slate-800/50'}`}
              >
                <p className={`text-xs font-semibold uppercase tracking-wider ${aktifGrupMu && kapali ? 'text-blue-400' : 'text-slate-500 group-hover:text-slate-400'}`}>
                  {groupName}
                </p>
                <ChevronDown className={`w-3 h-3 text-slate-600 group-hover:text-slate-400 transition-transform flex-shrink-0 ${kapali ? '-rotate-90' : ''}`} />
              </button>
              {!kapali && items.map(item => {
