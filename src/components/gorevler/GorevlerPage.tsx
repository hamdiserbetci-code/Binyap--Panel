...
const ONCELIK_COLORS: Record<string, string> = { dusuk:'bg-white/[0.06] text-slate-300', orta:'bg-blue-500/10 text-blue-400', yuksek:'bg-amber-500/10 text-amber-400', kritik:'bg-red-500/10 text-red-400' }
const ONCELIK_LABELS: Record<string, string> = { dusuk:'Düşük', orta:'Orta', yuksek:'Yüksek', kritik:'Kritik' }
const DURUM_COLORS: Record<string, string> = { beklemede:'bg-white/[0.06] text-slate-300', devam:'bg-blue-500/10 text-blue-400', tamamlandi:'bg-emerald-500/10 text-emerald-400' }
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <div>
          <h2 className="text-lg font-semibold text-white">Yapılacak İşler</h2>
      ...
      {hatirlatici.length > 0 && (
        <div className="bg-amber-500/10 border border-amber-200 rounded-xl px-4 py-3 mb-4 flex items-start gap-2">
      ...
      <div className="grid grid-cols-3 gap-3 mb-4">
        <div className="bg-white/[0.02] rounded-xl border border-white/[0.05] p-3">
          <p className="text-xs text-slate-400 mb-1">Toplam Görev</p>
          <p className="text-xl font-semibold text-white">{gorevler.length}</p>
        </div>
        <div className="bg-white/[0.02] rounded-xl border border-white/[0.05] p-3">
          <p className="text-xs text-slate-400 mb-1">Aktif</p>
          <p className="text-xl font-semibold text-blue-400">{aktif}</p>
        </div>
        <div className="bg-white/[0.02] rounded-xl border border-white/[0.05] p-3">
          <p className="text-xs text-slate-400 mb-1">Gecikmiş</p>
          <p className="text-xl font-semibold text-red-400">{gecikti}</p>
        </div>
      </div>

      <div className="flex gap-2 mb-4 flex-wrap">
        {[['hepsi','Tümü'],['beklemede','Beklemede'],['devam','Devam'],['tamamlandi','Tamamlandı']].map(([v,l])=>(
          <button key={v} onClick={()=>setFiltre(v)} className={`px-3 py-1.5 rounded-full text-xs font-medium ${filtre===v?'bg-blue-600 text-white':'bg-white/[0.02] border border-white/[0.08] text-slate-300'}`}>{l}</button>
        ))}
        <select value={filtreOncelik} onChange={e=>setFiltreOncelik(e.target.value)} className="ml-auto bg-white/[0.02] border border-white/[0.08] rounded-xl px-3 py-1.5 text-xs text-slate-300 outline-none">
      ...
      {loading ? <p className="text-center text-slate-400 py-8 text-sm">Yükleniyor...</p> :
        ...
        <div className="space-y-2">
          {filtered.map(g => {
            ...
            return (
              <div key={g.id} className={`bg-white/[0.02] rounded-xl border p-3.5 ${over?'border-l-4 border-l-red-400 border-y-slate-100 border-r-slate-100':bugun?'border-l-4 border-l-amber-400 border-y-slate-100 border-r-slate-100':'border-white/[0.05]'}`}>
                <div className="flex items-start gap-3">
                  <button onClick={()=>toggleDurum(g)} className={`w-5 h-5 rounded-full border-2 flex-shrink-0 mt-0.5 flex items-center justify-center transition-all ${g.durum==='tamamlandi'?'bg-emerald-500 border-emerald-500':g.durum==='devam'?'border-blue-400 bg-blue-500/10':'border-white/[0.1] hover:border-blue-400'}`}>
                  ...
                  <div className="flex-1 min-w-0">
                    <p className={`text-sm font-medium mb-1 ${g.durum==='tamamlandi'?'line-through text-slate-400':'text-white'}`}>{g.baslik}</p>
                    <div className="flex flex-wrap gap-1.5 items-center mb-1">
                      ...
                      {g.son_tarih && <span className={`text-[10px] flex items-center gap-1 ${over?'text-red-400':bugun?'text-amber-500':'text-slate-400'}`}>{over?<AlertTriangle size={10}/>:bugun?<Clock size={10}/>:null}{over?'Geçti: ':bugun?'Bugün: ':''}{g.son_tarih}</span>}
                    </div>
                    {g.yapilma_yuzdesi > 0 && (
        ...
        <Modal title={editing ? 'Görevi Düzenle' : 'Yeni Görev Ekle'} onClose={() => setModal(false)} size="lg"
          ...
          <div className="space-y-3">
            ...
            <label className="flex items-center gap-2 cursor-pointer">
              ...
              <span className="text-sm text-slate-200">Hatırlatıcı aktif et</span>
            </label>
          </div>
        </Modal>
      )}
