...
export default function GiderTakibiPage({ userId, firma }: Props) {
  ...
  const [form, setForm] = useState({ baslik:'', tutar:'', vade_tarihi:'', durum:'beklemede', aciklama:'', tur:'cari' })
  ...
  const fetchGiderler = useCallback(async () => {
    ...
    let query = supabase.from('odeme_plani').select('*')
      .eq('firma_id', firma.id).in('tur', ['alis', 'iscilik'])
  ...
  function openModal(g?: any) {
    ...
    setForm(g ? {
      ...
      durum: g.durum || 'beklemede', aciklama: g.aciklama || '', tur: g.tur || 'cari'
    } : { baslik:'', tutar:'', vade_tarihi:'', durum:'beklemede', aciklama:'', tur:'cari' })
    setModal(true)
  }
  ...
  const totalMaliyet = giderler.reduce((sum,g) => sum + (g.tutar || 0), 0)
  const totalMalatya = 0
  const toplam = totalMaliyet + totalMalatya
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4 gap-3 flex-wrap">
        <div>
          <h2 className="text-xl font-bold text-white">Gider Takibi</h2>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-4">
        <div className="bg-white rounded-xl border border-slate-100 p-4">
          <p className="text-xs text-slate-400">Toplam Maliyet</p>
          <p className="text-2xl font-bold text-red-500">{fmt(totalMaliyet)}</p>
        </div>
        <div className="bg-white rounded-xl border border-slate-100 p-4">
          <p className="text-xs text-slate-400">Malatya Gider</p>
          <p className="text-2xl font-bold text-violet-500">{fmt(totalMalatya)}</p>
        </div>
        <div className="bg-white rounded-xl border border-slate-100 p-4">
          <p className="text-xs text-slate-400">Toplam Gider</p>
          <p className="text-2xl font-bold text-rose-500">{fmt(toplam)}</p>
        </div>
      </div>
      ...
      {!selectedProje ? (
        ...
        <div className="bg-white/5 rounded-xl border border-slate-700 overflow-hidden">
          <div className="grid grid-cols-7 gap-2 bg-slate-900/70 text-slate-300 text-xs font-medium px-3 py-2">
            <div>Tarih</div><div>Açıklama</div><div className="text-right">Tutar</div><div>Tür</div><div>Durum</div><div>Ödeme</div><div>İşlemler</div>
          </div>
          {giderler.map((o:any) => (
      ...
      {modal && (
        <Modal title={editing ? 'Gideri Düzenle' : 'Gider Ekle'} onClose={() => setModal(false)}
          footer={
            <>
          ...
          <div className="space-y-3">
            <FormField label="Başlık" required>
              <input className={inputCls} value={form.baslik} onChange={e=>setForm({...form,baslik:e.target.value})} placeholder="Gider başlığı" />
            </FormField>
